...
cat("- MPE is negative, which tells us that the model is underestimating the predicted value than the actual value.")
```
...
$$
y_t = 1.1346 - 0.2218 \cdot y_{t-1} - 0.2829 \cdot y_{t-2} - 0.1260 \cdot y_{t-3} + 0.2241 \cdot y_{t-4} + 0.2442 \cdot y_{t-5} + 0.1892 \cdot e_{t-1} + 4.7544 \cdot covariate + e_t
$$
...
```{r, echo=FALSE}
cat("MAE value for regARIMA without cross-validation: 6.880539")
cat("MAE value for regARIMA with cross-validation: 8.950738")
cat("MAE for VAR without cross-validation: 13.68")
cat("MAE for VAR with cross-validation: 683.4063")
```
<br> </br>
...
cat("Forecasted for VAR with cross-validation: 2102.099")
```
